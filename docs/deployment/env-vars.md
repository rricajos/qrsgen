...
|---|---|---|
| `QRSGEN_VERSION` | `0.29.0` | Tag de imagen Docker (`qrsgen:${QRSGEN_VERSION}`). |
| `POSTGRES_PORT` | `5432` | |
| `POSTGRES_DB` | `bridge` | |
...
| `QRSGEN_GROUP_PREFIX_SENDER` | `true` | Si `true`, mensajes incoming de grupos se postean a downstream con prefijo `+<phone> - <name>:\n<body>` para distinguir participantes en una misma conv. Pon a `false` si tu integración parsea el body raw. Desde v0.29.0. |
| `DEDUP_ENABLED` | `true` | |
