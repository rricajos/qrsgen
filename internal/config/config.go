...
// NOTA v0.64.7: las envs `CHATWOOT_DB_URL`, `QRSGEN_BACKDATE_INTERVAL`,
// `QRSGEN_BACKDATE_BATCH_SIZE` y `QRSGEN_BACKDATE_TOLERANCE_SEC` que
// existían en v0.54.0-v0.64.6 han sido eliminadas. El backdate worker
// pasó al integration repo (n8n workflow `OMNIA_BACKDATE`) para
// mantener qrsgen agnóstico del downstream. qrsgen sigue emitiendo
// `external_created_at` en cada POST como siempre; la responsabilidad
// de aplicar ese timestamp al `created_at` del downstream queda del
// lado del integrador.
